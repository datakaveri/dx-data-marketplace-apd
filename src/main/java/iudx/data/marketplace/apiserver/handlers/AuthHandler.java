...
public class AuthHandler implements Handler<RoutingContext> {
    ...
    authenticator.tokenIntrospect(
        ...
        handler -> {
          if (handler.succeeded()) {
            ...
            getUserInfo(tokenIntrospectResult)
  ...
  private Future<User> getUserInfo(JsonObject tokenIntrospectResult) {
    ...
    JsonObject params = new JsonObject().put("$1", tokenIntrospectResult.getString("userId"));
    postgresServiceImpl.executePreparedQuery(
        GET_USER,
        params,
        handler -> {
          if (handler.succeeded()) {
  ...
  /**
   * Returns normalised path without ID <br>
