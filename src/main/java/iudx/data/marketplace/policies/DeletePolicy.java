...
public class DeletePolicy {
  ...
  public DeletePolicy(PostgresqlService postgresService, AuditingService auditingService, Api api) {
  ...
  public Future<JsonObject> executeUpdateQuery(String query, UUID policyUuid, User user) {
   ...
   Future<JsonObject> future = postgresService
        .executePreparedQuery(query, param)
        .compose(
            result -> {
    ...
    if(future.failed() && !future.cause().getMessage().contains(BAD_REQUEST.getUrn()))
    {
  ...
  public Future<Boolean> verifyPolicy(User user, String query, UUID policyUuid) {
   ...
   Future<Boolean> future = postgresService
        .executePreparedQuery(query, param)
        .compose(
            dbResult -> {
              if (dbResult.getJsonArray(RESULTS).isEmpty()) {
                ...
                return Future.failedFuture(failureResponse.encode());
              }
   ...
   {
       LOG.error("Failure while executing the query : {}", future.cause().getMessage());
       JsonObject response =
               new JsonObject()
                       .put(TYPE, HttpStatusCode.INTERNAL_SERVER_ERROR.getValue())
                       .put(TITLE, ResponseUrn.DB_ERROR_URN.getUrn())
