...
public class PostgresqlServiceImpl implements PostgresqlService {
  private static final Logger LOGGER = LogManager.getLogger(PostgresqlServiceImpl.class);

  private final PgPool client;
  public PostgresqlServiceImpl(final PgPool pgclient) {
  ...
  //TODO: Remove the respbuilder, directly send error | failure message OR results in a json object
  public Future<JsonObject> executeCountQuery(
    ...
    client
        ...
        .onFailure(
            failureHandler -> {
                LOGGER.error("Failure : {}",failureHandler.getMessage() );
//                String response =
//                  new RespBuilder()
//                      .withType(ResponseUrn.DB_ERROR_URN.getUrn())
//                      .withTitle(ResponseUrn.DB_ERROR_URN.getMessage())
...
//                      .getResponse();
//              promise.fail(response);
              String failureMessage = new JsonObject()
                      .put(DETAIL, failureHandler.getLocalizedMessage()).encode();
