...
import static iudx.data.marketplace.authenticator.authorization.Api.PROVIDER_LIST_PURCHASES;
import static iudx.data.marketplace.authenticator.authorization.Method.*;

public class DelegateAuthStatergy implements AuthorizationStatergy {

  private static final Logger LOGGER = LogManager.getLogger(DelegateAuthStatergy.class);

  static List<AuthorizationRequest> delegateAuthRules = new ArrayList<>();
  ...
  public boolean isAuthorized(AuthorizationRequest authorizationRequest, JwtData jwtData) {
    ...
    return delegateAuthRules.contains(authorizationRequest);
  }
}
