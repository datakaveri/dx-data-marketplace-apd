...
public class VariantServiceTest {
  ...
  public void testUpdateVariant(VertxTestContext testContext) {
    ...
    request.put(PRODUCT_ID, UUID.randomUUID().toString());
    request.put(PRODUCT_VARIANT_NAME, UUID.randomUUID().toString());
//    request.put(RESOURCES_ARRAY, new JsonArray().add(0, new JsonObject().put(ID, UUID.randomUUID())));
    request.put(RESOURCES_ARRAY, jsonArrayMock);
    when(jsonObjectMock.mergeIn(any())).thenReturn(jsonObjectMock);
    when(jsonArrayMock.encode()).thenReturn("Some string");
    when(provider.getUserId()).thenReturn("Some ID");
    when(jsonObjectMock.getString("status")).thenReturn("active");
    when(jsonObjectMock.getString("provider_id")).thenReturn("Some ID");
    variantServiceSpy
        .updateProductVariant(provider, jsonObjectMock)
        .onComplete(
            handler -> {
              if (handler.succeeded()) {
                verify(variantServiceSpy, times(1))
                    .updateProductVariantStatus(anyString(), anyString());
                testContext.completeNow();
              } else {
  ...
  public void testListProductVariantsWhenDbExecutionFailed(VertxTestContext vertxTestContext) {
    ...
    JsonArray jsonArray = new JsonArray();
    request.put("productId", "someDummyProductId").put("resourceServerUrl", "abcd");
    when(postgresService.executePreparedQuery(anyString(), any())).thenReturn(Future.failedFuture(throwable));
